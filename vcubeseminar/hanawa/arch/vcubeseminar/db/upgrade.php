...
function xmldb_vcubeseminar_upgrade($oldversion) {
    ...
    if ($oldversion < 2016060100) {
        ...
        //create table 'vcubeseminar_domain'
        $table = new xmldb_table('vcubeseminar_domain');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
    	$table->add_field('alias', XMLDB_TYPE_CHAR,'512',null,null,null,null);
        $table->add_field('vcseminar_domain', XMLDB_TYPE_CHAR, '128', null, null, null, null);
        $table->add_field('vcseminar_id', XMLDB_TYPE_CHAR, '128', null, null, null, null);
        $table->add_field('vcseminar_password', XMLDB_TYPE_CHAR, '128', null, null, null, null);
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        if(!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
    	...
    	//add vcubeseminardomainid
        $field = new xmldb_field('vcubeseminardomainid', XMLDB_TYPE_INTEGER, '10', null, null, null, null);
    	if (!$dbman->field_exists($table, $field)) $dbman->add_field($table, $field);
