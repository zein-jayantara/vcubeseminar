...
class vcseminar{
	 ...
	 */
	public function get_domain_list($domain_id = null){
		global $DB;
        if($domain_id == null) {
            return $DB->get_records('vcubeseminar_domain');
        }
        else {
            return $DB->get_records('vcubeseminar_domain', array('id' => $domain_id));
        }
	}
	 ...
	 * @return array $room 部屋の一覧
	 */
	public function get_room_list_($domain = null, $seminarid = null, $seminarpass = null){
		$ret = $this->execute_api('','/api/atom/room', 'GET', $domain, $seminarid, $seminarpass);
	 ...
	 * @return array $room 部屋の一覧
	 */
	public function get_room_list($domain, $seminarid, $password){
		$rooms = array();

        // Make URL
        $url = $domain.'/api/atom/room';

        // Make X-WSSE String
        $created  = date('Y-m-d\TH:i:s\Z');
        $nonce    = sha1(uniqid( rand(), true)); // Number used once
        $enonce   = base64_encode( $nonce ); // Encoded nonce
        $epdigest = base64_encode(sha1($nonce.$created.sha1($password))); // Encoded password digest
        $xwsse    = <<< WSSE
X-WSSE: UsernameToken Username="{$seminarid}", PasswordDigest="{$epdigest}", Nonce="{$enonce}", Created="{$created}"
WSSE;

        // cURL Setting
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array($xwsse));
		...
		if($result !== false) {
            foreach(simplexml_load_string($result)->room as $room) {
                $rooms[$room->room_key->__toString()] = array(
                    'id' => $room->room_key->__toString(),
                    'name' => $room->room_name->__toString(),
                );
            }
        }
		return $rooms;
	 ...
	 */
	private function execute_api($param, $apiurl = '', $method = 'GET', $domain = null, $seminarid = null, $seminarpass = null){
//        print_r($apiurl);
//        echo "<br />";
//
		try{
            ...
            if($domain === null) {
                $domain = $this->{$this->accountparam[0]};
            }
            if($seminarid === null) {
                $seminarid = $this->{$this->accountparam[1]};
            }
            if($seminarpass === null) {
                $seminarpass = $this->{$this->accountparam[2]};
            }
			...
			$pdigest  = sha1( $nonce . $created . sha1($seminarpass) );
			$nonce    = base64_encode( $nonce );
			$pdigest  = base64_encode( $pdigest );
			$tmp = <<< WSSE
X-WSSE: UsernameToken Username="{$seminarid}", PasswordDigest="{$pdigest}", Nonce="{$nonce}", Created="{$created}"
WSSE;
			$x_wsse = array($tmp);
			$url = $domain.$_apiurl;
			$ch=curl_init($url);
...
SQL;
		...
		if ($ret === false){
			...
			//ブロックモジュールがある時
			$rawconfigdata = (array)unserialize(base64_decode($ret->configdata));
            $configdata = array();
            foreach($rawconfigdata as $strdomain => $active) {
                $domain_keys = explode('_', $strdomain);
                $configdata[$domain_keys[0]][$domain_keys[1]] = $active;
            }
        ...
        foreach($domains as $domain) {
            $rooms[$domain->id] = $this->get_room_list($domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
            foreach($rooms[$domain->id] as $roomid => $roomdata) {
                switch ($setting_flag) {
                    case 0:
                    case 1:
                        if ( $block_setting == 0){
                            //すべての部屋の一覧
                            $allow_rooms[$domain->id.'_'.$roomdata['id']] = $domain->alias . ' ' . $roomdata['name'];
                        }
                    break;
                    case 2:
                        if(isset($configdata[$domain->id][$roomdata['id']]) && $configdata[$domain->id][$roomdata['id']]) { //利用を許可された部屋の一覧
                            $allow_rooms[$domain->id.'_'.$roomdata['id']] = $domain->alias . ' ' . $roomdata['name'];
                        }
                    break;
                }
 ...
 */
	...
	function set_new_reserve($data){
        ...
        $domains = $this->get_domain_list($data->vcubeseminardomainid);
        if(!$domains[$data->vcubeseminardomainid]) {
            return false;
        }
		$param = $this->make_xml($tmp);
		$ret_xml = $this->execute_api($param, '/api/atom/reserve', 'POST', $domains[$data->vcubeseminardomainid]->vcseminar_domain, $domains[$data->vcubeseminardomainid]->vcseminar_id, $domains[$data->vcubeseminardomainid]->vcseminar_password);
		$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
        $seminar_key = "";
        if(isset($xml->seminar_key)) {
	...
	function update_reverve($data){
        ...
        $domain = $domains[$data->vcubeseminardomainid];

		$param = $this->make_xml($tmp);
		$ret_xml = $this->execute_api($param, '/api/atom/reserve/'.$data->seminarkey, 'PUT', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
		$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
	}
	...
	function delete_seminar($data){
		...
		if($status!==0 && $status!==4){
			...
			$ret_xml = $this->execute_api(null, '/api/atom/reserve/'.$data->seminarkey, 'DELETE', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
			$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
		}
	}
	 ...
	 */
	function get_seminar_info($seminar_key, $domain_id){
        $domains = $this->get_domain_list($domain_id);
        if(!$domains[$domain_id]) {
            return false;
        }
        $domain = $domains[$domain_id];
		$ret_xml = $this->execute_api('', '/api/atom/reserve/'.$seminar_key, 'GET', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
		$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
		return $xml;
	}
	 ...
	 */
	function get_allow_room_seminar($domain_key, $room_key){
		$seminar_ary = array();

        $domain_id = null;
        $vcseminar_domain = null;
        $vcseminar_id = null;
        $vcseminar_password = null;
        $domains = $this->get_domain_list($domain_key);
        foreach($domains as $domain) {
            if($domain->id == $domain_key) {
                $domain_id = $domain->id;
                $vcseminar_domain = $domain->vcseminar_domain;
                $vcseminar_id = $domain->vcseminar_id;
                $vcseminar_password = $domain->vcseminar_password;
            }
        }
		$ret_xml = $this->execute_api(array('room_key'=>$room_key),'/api/atom/ondemand', 'GET', $vcseminar_domain, $vcseminar_id, $vcseminar_password);
	...
	function is_set_seminar($data){
		...
		$ret_xml = $this->execute_api($tmp, '/api/atom/reserve', 'GET', $domains[$data['vcubeseminardomainid']]->vcseminar_domain, $domains[$data['vcubeseminardomainid']]->vcseminar_id, $domains[$data['vcubeseminardomainid']]->vcseminar_password);
		$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
        $count = 0;
        if(isset($xml->count)) {
            $count = $xml->count->__toString();
 ...
 */
	...
	function get_seminar_url($vcdata, $teacher_flag = 0){
		...
		if ( $teacher_flag == 0 ){
			...
			//教師
			$ret_xml = $this->get_seminar_info($vcdata->seminarkey, $vcdata->vcubeseminardomainid);
			if( isset($ret_xml->count) && $ret_xml->count->__toString() != 0 ){
				//DBへの保存
	...
	protected function get_student_url($vcdata){
        ...
        $domains = $this->get_domain_list($vcdata->vcubeseminardomainid);
        if(!$domains[$vcdata->vcubeseminardomainid]) {
            return false;
        }
        $domain = $domains[$vcdata->vcubeseminardomainid];

		$ret_xml = $this->execute_api($param, '/api/atom/participant', 'POST', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
		$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
		return $xml;
	}
	...
	function set_password($vcdata, $password){
        ...
        $domain = $domains[$vcdata->vcubeseminardomainid];
		$ret_xml = $this->execute_api($param, '/api/atom/ondemand/'.$vcdata->seminarkey, 'PUT', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
		$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
	}
	...
	function get_minutes($vcdata){
		...
		if($ret === false){
			//URLが取得可能か判断
			$ret_xml = $this->execute_api('', '/api/atom/ondemand/'.$vcdata->seminarkey, 'GET', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
			$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
...
HTML;
	...
	function get_entering_log($vcdata, $tflag = 0, $start = 0, $pagelimit = 10, $page_flag = 0){
		...
		$ret_xml = $this->execute_api('', '/api/atom/audiencelog/'.$param, 'GET', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
		$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
		if($page_flag == 0){
			return $xml;
		}else{
...
HTML;
	...
	function output_ondemandstatus($seminar){
        ...
        $domains = $this->get_domain_list($seminar->vcubeseminardomainid);
        if(!$domains[$seminar->vcubeseminardomainid]) {
            return false;
        }
        $domain = $domains[$seminar->vcubeseminardomainid];

		$ret_xml = $this->execute_api($tmp, '/api/atom/reserve/'.$seminar_key, 'GET', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
		$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
	}
	...
	function ishold($seminar){
		...
		$ret_xml = $this->execute_api($tmp, '/api/atom/reserve/'.$seminar_key, 'GET', $domains[$seminar->vcubeseminardomainid]->vcseminar_domain, $domains[$seminar->vcubeseminardomainid]->vcseminar_id, $domains[$seminar->vcubeseminardomainid]->vcseminar_password);
		$xml = $this->check_error($ret_xml); //データ取得＆エラーチェック
        if(!isset($xml->seminar)) {
            return false;
        }
	...
	function add_documents($data,$filelist){
		foreach ($filelist AS $file){
			...
			if($file->type == 0) $ret = $this->add_whiteboard($file,$data->seminarkey, $data);
			else if($file->type == 1) $ret = $this->add_filecabinet($file,$data->seminarkey, $data);
			else if($file->type == 2){
				$ret = $this->add_banner($file,$data->seminarkey, $data);
				if($file->link_url!='') $this->add_bunnerURL($file,$data->seminarkey,$ret, $data);
			}
	 ...
	 * @return mixed
	 */
	function add_whiteboard($file,$seminarkey, $vcdata = null){
        ...
        $domain = $domains[$vcdata->vcubeseminardomainid];
		$ret = $this->execute_api($post, '/api/atom/whiteboard', 'POST', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
	 ...
	 * @return mixed
	 */
	function add_filecabinet($file,$seminarkey,$vcdata=null){
		$b64file = base64_encode($file->filedmp);
		$post = '
			<?xml version="1.0" encoding="utf-8"?>
			<entry xmlns="http://www.w3.org/2005/Atom">
        ...
        $domain = $domains[$vcdata->vcubeseminardomainid];
		$ret = $this->execute_api($post, '/api/atom/cabinet', 'POST', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
		$xml = $this->check_error($ret);
	 ...
	 * @return mixed
	 */
	function add_banner($file,$seminarkey,$vcdata=null){
		$b64file = base64_encode($file->filedmp);
		$post = '
			<?xml version="1.0" encoding="utf-8"?>
			<entry xmlns="http://www.w3.org/2005/Atom">
        ...
        $domain = $domains[$vcdata->vcubeseminardomainid];
		$ret = $this->execute_api($post, '/api/atom/banner', 'POST', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
		$xml = $this->check_error($ret);
	 ...
	 */
	function add_bunnerURL($file,$seminarkey,$storage_key,$vcdata=null){
		$apiurl = '/api/atom/banner/';
		$docs = $this->action_get_document($seminarkey,$apiurl,$vcdata);
		foreach($docs->banner as $doc){
			...
			if($doc->storage_key == $storage_key){
				...
				if($s_key){
                    ...
                    $domain = $domains[$vcdata->vcubeseminardomainid];
					$ret = $this->execute_api($post, '/api/atom/banner', 'PUT', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
					$xml = $this->check_error($ret);
		...
		if($filedata){
			...
			foreach ($filedata AS $file){
				if($withfilepicker){
					...
					$this->action_delete_document($data->seminarkey,$file->documentid,$apiurl,$data);
				}
			}
			//moodleからの削除
	 ...
	 * @throws moodle_exception
	 */
	function action_delete_document($seminarkey,$documentid,$apiurl,$vcdata){
		$files = $this->action_get_document($seminarkey,$apiurl,$vcdata);
        $domains = $this->get_domain_list($vcdata->vcubeseminardomainid);
        if(!$domains[$vcdata->vcubeseminardomainid]) {
            return false;
        }
			...
			foreach ($files->folder as $file){
				if($file->file->storage_key == $documentid){
					...
					if($s_key){
						...
						$ret = $this->execute_api($post='', $url,'DELETE', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
						$xml = $this->check_error($ret);
					}
					break;
				}
	 ...
	 */
	function action_get_document($seminarkey,$apiurl,$vcdata=null){
		...
		$ret = $this->execute_api($post='', $url,'GET', $domain->vcseminar_domain, $domain->vcseminar_id, $domain->vcseminar_password);
		$xml = $this->check_error($ret);
		if(!$xml['error_msg']){
			return $xml;
		}else{
