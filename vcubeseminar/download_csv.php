...
//ログの総数を取得
$totalcount = $obj->get_entering_log($vcubeseminar, $tflag, 0, 0, 1);

//ログを取得
$logdata = $obj->get_entering_log($vcubeseminar, $tflag, 0, $totalcount);
$logdata = $obj->make_tabledata($vcubeseminar, $logdata, $tflag);
...
$header1[] = get_string('start_datetime', 'vcubeseminar');
$header1[] = date('Y-m-d H:i', $vcubeseminar->starttime-$offset);
$header1[] = get_string('curtaintime', 'vcubeseminar');
$header1[] = date('Y-m-d H:i', $vcubeseminar->curtaintime-$offset);
$header1[] = get_string('end_datetime', 'vcubeseminar');
$header1[] = date('Y-m-d H:i', $vcubeseminar->endtime-$offset);
$header1[] = get_string('timezone', 'vcubeseminar');
...
$header2[] = get_string('entry_datetime', 'vcubeseminar');
$header2[] = get_string('leave_datetime', 'vcubeseminar');
$header2[] = get_string('teacher_flag', 'vcubeseminar');
$data .= '"'.implode('","', $header2).'"'."\r\n";

$i = 1;
while(list($key, $value) = each($logdata)){
	$timezoneobj->unixTimeToUserTime($value['enter']);
	$timezoneobj->unixTimeToUserTime($value['leave']);
	$tmp = array();
	$tmp[] = $i++;
	$tmp[] = $value['username'];
	$tmp[] = $value['lastname'].' '.$value['firstname'];
	$tmp[] = $value['email'];
	$tmp[] = date('Y/m/d H:i:s', $value['enter']);
	$tmp[] = date('Y/m/d H:i:s', $value['leave']);
	$tmp[] = ($value['username'] != '' )? '':get_string('tflag', 'vcubeseminar');
